<template>
  <div class="home-wrap">
    <div class="home_header">三维矩阵之总是看向特定的F且相机又在绕一圈F旋转的相机</div>
    <div>
      <h4>tips</h4>
      <ul>
        <li>相机：看向原点，在半径为 radius * 1.5 的圆上移动的相机</li>
        <li>利用mat4.lookAt方法，我们可以快速的得到眼睛从(10, 10, 10)点，望向(0, 0, 0)点的所需要的反向几何变换矩阵，我们把所得到的矩阵称为视图变换矩阵</li>
      </ul>
    </div>
...
export default {
  ...
  methods:{
    ...
    async setUMatrix() {
      ...
      // 2、计算相机矩阵
      // 计算第一个 F 的位置
      let fPosition = [this.radius, 0, 0];
      // 计算相机在圆上的位置矩阵
      let cameraMatrix = mat4.create();
      mat4.rotateY(cameraMatrix, cameraMatrix, glMatrix.toRadian(this.sliderBar.cameraAngle));
      ...
      mat4.lookAt(cameraLookMatrix, cameraPosition, fPosition, up);
