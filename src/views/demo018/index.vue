<template>
  <div class="home-wrap">
    <div class="home_header">三维点光源</div>
    <div>
      <h4>tips</h4>
      <ul>
        <li>每个点都有一个不同的面到光源的矢量， 将这个矢量和法向量点乘后，表面上的每个点都会有一个不同的光照值</li>
...
export default {
  ...
  methods:{
    ...
    async setUMatrix() {
      ...
      // 计算相机的朝向矩阵,根据相机矩阵创建模型视图矩阵--都在lookAt方法中完成
      ...
      // 2.设置平移矩阵
      this.modelMatrix = mat4.translate(this.modelMatrix, this.modelMatrix, [this.sliderBar.xVal, this.sliderBar.yVal, this.sliderBar.zVal]);
      // 3.设置旋转矩阵
      this.modelMatrix = mat4.rotateX(this.modelMatrix, this.modelMatrix, glMatrix.toRadian(this.sliderBar.angleXVal))
      this.modelMatrix = mat4.rotateY(this.modelMatrix, this.modelMatrix, glMatrix.toRadian(this.sliderBar.angleYVal))
      this.modelMatrix = mat4.rotateZ(this.modelMatrix, this.modelMatrix, glMatrix.toRadian(this.sliderBar.angleZVal))
      // 4.设置缩放矩阵
      this.modelMatrix = mat4.scale(this.modelMatrix, this.modelMatrix, [this.sliderBar.scaleXVal, this.sliderBar.scaleYVal, this.sliderBar.scaleZVal]);
     ...
     // 2.设置平移矩阵
      ...
      let uWorldViewLocation = this.gl.getUniformLocation(this.shaderProgram, 'u_worldViewProjection');
      // 设置u_worldViewProjection的值
      this.gl.uniformMatrix4fv(uWorldViewLocation, false, this.worldViewProjectionMatrix);
      ...
      // 获取u_world在着色器程序中的位置
      let uWorldLocation = this.gl.getUniformLocation(this.shaderProgram, 'u_world');
      // 设置u_worldViewProjection的值
      this.gl.uniformMatrix4fv(uWorldLocation, false, this.modelMatrix);
    ...
    async setColors() {
      ...
      let lightWorldPositionLocation =
          this.gl.getUniformLocation(this.shaderProgram, "u_lightWorldPosition");
      this.gl.uniform3fv(lightWorldPositionLocation, [120, 130, 160]);
    },
    /**
     * 设置字母F的缓冲数据（屏幕像素坐标）
     * @returns {Promise<void>}
